...
\begin{enumerate}
  ...
  \item Chamfering the staircase with triangles (the proposal of these
        ...
        of the benefit (N.6).
  \item The effective fix for $p=4$ is \emph{truncating the Taylor
        extrapolation order} $k$ on the offending faces: $k=2$ restores
        definiteness at step cells up to $s \approx 0.75$, and $k=1$
        restores it everywhere with a wide margin (N.7). Combining
        chamfer (which caps $s$ at $\approx 0.7$) with $k=2$ is the
        predicted robust $p=4$ configuration that keeps $O(h^3)$ boundary
        consistency.
\end{enumerate}
...
\begin{itemize}
  ...
  \item $k=2$ restores definiteness for $p=4$ up to $s \approx 0.75$ and
        for $p=3$ everywhere. Combined with the chamfer, which caps $s$
        at $\approx 0.7$ \emph{and} eliminates two-face cells, $k=2$
        extrapolation is the predicted robust $p=4$ configuration, with
        $O(h^3)$ boundary consistency --- the accuracy loss relative to
        $k=p$ appears only in a boundary layer and should be checked
        against the interior $O(h^{p+1})$ rates.
  \item A shift-adaptive rule ($k = p$ where $s \le 0.4$, $k = 2$ where
        $0.4 < s \le 0.75$, $k=1$ beyond) follows directly from the
        tables and costs one \texttt{if} per face.
...
$p=3$ uniformly robust; (b) chamfer $+$ $k=2$ $\Rightarrow$ $p=4$ robust;
(c) shift-adaptive $k$ alone (no chamfer) already recovers $p=4$ with the
existing smoother --- this is a ten-line change in the extrapolation
routine and the highest-information-per-effort experiment available;
