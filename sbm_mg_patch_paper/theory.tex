...
\emph{Remark (generality).} The construction transfers to any operator for
which the surrogate face is non-characteristic, i.e.\ the PDE can be solved
for the highest normal derivative: variable-coefficient elliptic (divide by
$n^T\!A\,n > 0$; the margin degrades with anisotropy $\|A\|/(n^T\!A\,n)$),
elasticity (acoustic tensor $n^T C n$ invertible by Legendre--Hadamard),
parabolic (imports a $\partial_t u$ term, mass-like entries in implicit
schemes), and Stokes (momentum supplies $\nu\partial_n^2\mathbf u$ via
$\nabla p - \mathbf f$; incompressibility gives $\partial_n u_n =
-\mathrm{div}_{tan}\mathbf u_{tan}$ for free; $\Delta p = \mathrm{div}\,
\mathbf f$ closes the pressure recursion). For first-order hyperbolic
systems this is exactly the inverse Lax--Wendroff procedure of Tan \& Shu,
introduced for the same reason: direct high-order boundary extrapolation is
unstable, PDE-substituted extrapolation is not. Costs that transfer:
$k-1$ derivatives of the data (and coefficients) are needed near the
boundary, and the definiteness \emph{margins} (unlike the mechanism) are
operator-specific --- the certificates of N.4 should be recomputed per
operator.
...
\subsection*{N.7c Symmetry: a two-out-of-three obstruction and a geometric way out}

The nonsymmetry of SBM is structural: integration by parts forces the
\emph{plain} trace $v$ in the consistency term $-\int(\partial_n u)\,v$,
while symmetry would demand $\mathcal E v$ there. The options split as
follows.

\paragraph{Lemma N.6 (matched pairing is unconditionally coercive).}
\emph{Define the symmetric variant}
\[
  b(u,v) = \int_K \nabla u\cdot\nabla v
  - \int_F (\partial_n u)\,\mathcal E v
  - \int_F (\partial_n v)\,\mathcal E u
  + \sigma \int_F \mathcal E u\, \mathcal E v .
\]
\emph{Then $b(v,v) \ge \tfrac12\|\nabla v\|^2 + \tfrac{\sigma}{2}\|\mathcal
E v\|_F^2$ for $\sigma \ge 4 C_{tr} p^2/h$, independently of the shift, the
polynomial degree, and the extension.}
\emph{Proof:} Young's inequality,
$2|\int(\partial_n v)\mathcal E v| \le \tau \|\partial_n v\|_F^2 +
\tau^{-1}\|\mathcal E v\|_F^2$ with $\tau = h/(2C_{tr}p^2)$; the trace
inverse inequality absorbs the first term into $\|\nabla v\|^2$. The
extension appears \emph{identically} in the cross term and the penalty, so
the Chebyshev amplification cancels from the argument. $\square$

The entire pathology of N.4 disappears --- CG applies, additive smoothers
are rehabilitated --- but the symmetrization defect relative to the
consistent form,
$-\int_F (\partial_n u)(\mathcal E v - v) = O(\delta)$, caps the boundary
accuracy at first order. Useful as an \emph{SPD local fallback} on the
worst faces (an alternative to $k$-truncation with better solver
structure), not as the global high-order method.

\paragraph{Order-by-order correction hits a wall.} The leading defect
$\delta \int_F (\partial_n u)(\partial_n v)$ is itself symmetric;
subtracting it restores $O(\delta^2)$ consistency while preserving
symmetry, and the cascade can be continued. But the subtracted term
contributes $-\delta\|\partial_n v\|_F^2 \sim -C\,p^2 s\,\|\nabla v\|_K^2$
to the diagonal: coercivity is lost at exactly $p^2 s \sim 1$ --- the same
threshold as before in a different guise. Empirically then: of
\emph{\{symmetric, high-order consistent, coercive from cell-local
data\}} one can have any two but not all three; this is Lemma N.5's
obstruction recurring, and the same three escapes (PDE, data, support)
apply. Whether PDE-substituting the correction terms
($\partial_n^2 \to$ tangential) preserves both symmetry and definiteness
is open and directly decidable with the certificate script.

\paragraph{The geometric route (recommended for a symmetric method):
shift the mesh, not the boundary condition.} Deform the boundary cells so
the surrogate boundary lands on $\Gamma$ (isoparametric unfitted FEM in
the spirit of Lehrenfeld): standard \emph{symmetric} Nitsche then applies
with no extension operator at all --- symmetry and high order obtained
through geometry instead of algebra, the exact dual of ``shifted
boundary''. The staircase cannot be smoothly deformed (corners), but the
\emph{chamfered} surrogate can: it is a Lipschitz polygon at distance
$O(h)$ from $\Gamma$ with interior angles near $\pi$, and an $O(h)$
piecewise-polynomial blend maps it onto $\Gamma$ with uniformly bounded
Jacobians. In this light the chamfer is not merely a $p=3$ patch: it is
the enabler of a fully symmetric high-order discretization. Cost:
non-affine element maps in a one-cell boundary layer (partial loss of
tensor-product structure there), standard fitted-Nitsche analysis
otherwise.

\paragraph{Other symmetric options.} A Lagrange-multiplier/mortar
treatment of the Dirichlet condition yields a symmetric saddle point ---
heavier machinery, but with solver structure (Vanka-type patch smoothers)
well within this project's toolbox. Least-squares symmetrization is an
anti-recommendation: it squares the operator and hence the amplification
($T_p^2 \to T_p^4$).
...
% Tan & Shu, JCP 2010 (inverse Lax-Wendroff boundary treatment, for N.7b(A)).
% Gibou & Fedkiw (ghost-point interpolation, for N.7b(B)).
% Badia, Verdugo, Martin (AgFEM aggregated extension, for N.7b(C)).
