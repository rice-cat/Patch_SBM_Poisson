...
>
> **Note on Author Name Update:** We would like to inform the editor regarding a correction to the author name "Ajay Ajith", which has been updated to "Ajith Ajay" in the revised manuscript. Following South Indian naming conventions, "Ajay" is the name he wants to used in references and "Ajith" is the patronymic. To ensure that future academic citations correctly refer to his given name ("work by Ajay [X]"), we have swapped the order of the names.
...
## Reviewer #2:
 ...
 > We thank the reviewer for this positive assessment. We appreciate all constructive suggestions, which we have carefully addressed below to further strengthen the paper.
 ...
 > We thank the reviewer for the constructive feedback regarding the methodological details and reproducibility. We agree that a clearer algorithmic description enhances the paper's accessibility and didactic value. To address this, we have significantly expanded the methodology section by adding a new, self-contained subsection titled "Algorithm for Patch Generation." This section now includes two detailed algorithms (the main routine and `CollectFullResidualDoFs`) that closely mirror our implementation. More importantly, we have publicly released the complete source code.
...
5. I'd suggest to refer to the $\lambda = 0.5$ as the optimal surrogate boundary approach. Besides, the authors say that this may lead to ill conditioning. I'd like to ask the authors about this statement and to be more specific about it in the manuscript.
    > We updated the text to refer to $\lambda=0.5$ as the "optimal surrogate boundary approach". We also refined our language to clarify that the continuous system remains formally well-posed; the difficulties are purely algebraic rather than true ill-conditioning. Specifically, negative shifts lead to complex eigenvalues with non-zero imaginary parts for higher polynomial degrees (as observed in [34]), which causes issues for the iterative solver and smoothers.
...
6. I would provide a more rigorous description of the high-order Taylor expansion considering that the paper targets order higher than one. Besides, I'd also include the truncation error in the definition.
    > We refined the high-order Taylor expansion definition to explicitly include the polynomial summation form up to degree $k$ and formally define the $\mathcal{O}(\|\mathbf{d}\|^{k+1})$ truncation error.
...
9. As far as I understand from the paragraph right after Eq. 4, the values in the true boundary $\Gamma$ are computed by direct evaluation in order to skip high order derivatives. Does this mean that the values are interpolated in $\Gamma$ by using the discretization of the intersected elements? If so, this would imply to add all the DOFs of the intersected cells (not only those attached to the surrogate boundary) to the linear system but without performing the integration of such cells, something that, if I am not wrong, would result in zero entries in the left hand side matrix and thus in an ill-conditioned problem. I think that the authors should better explain this.
    > We added a clarification after Equation 4. Values at the true boundary are computed using the discretization of active surrogate cells. We map points on the true boundary to the reference unit cell of their associated active element, which may evaluate slightly outside $[0,1]^d$. This polynomial extrapolation avoids explicit derivative computations without requiring additional DOFs for non-active cells.
...
12. Section 4.1. Which is the motivation to add all the cells' DOFs to the linear system and get an ill-conditioned problem. I would understand paying this price in the case of moving boundaries to avoid resetting the sparse matrix graph each time $\Gamma$ changes but I do not see the point in this case. Can the authors clarify this?
    > We clarified this in Section 4.1. Keeping DOFs for all cells simplifies the implementation and maintains a consistent sparsity pattern, allowing us to reuse standard multigrid transfer operators from \texttt{deal.II}. Zero rows are naturally handled by GMRES and constrained on the coarsest level. In a fully optimized code, inactive cell DOFs could easily be minimized by local refinement.
...
13. Section 4.2. The idea described in "The extrapolation of the function values..." is not clear at all for me. I think this is very much related to my observation 9. Can the authors clarify this.
    > We rephrased this paragraph in Section 4.2 to align with our response to Comment 9. We now explicitly state that direct polynomial extrapolation is achieved by evaluating shape functions at projected points on the true boundary $\Gamma$, avoiding manual derivative computations.
...
16. Though explained in [34], I think that the readers would appreciate a more detailed remark on the fact that using the optimal surrogate boundary (i.e., $\lambda=0.5$) results in negative eigenvalues that affect the convergence as, a priori, one may easily think that $\lambda=0.5$ is always the best.
    > We expanded this discussion in the numerical results section. We clarified that $\lambda=0.5$ generates shifts pointing inwards to the true domain, which leads to complex eigenvalues with non-zero *imaginary* (not negative) parts for higher-order elements [34]. This behavior disrupts the multigrid smoother.
...
On the basis of that, I recommend publication after a minor revision according to the comments below.
 ...
 > We sincerely thank the reviewer for their careful reading of our manuscript and highly encouraging summary of our work. We certainly won't disagree with this assessment. We greatly appreciate your constructive suggestions, which we have carefully addressed below to further strengthen the paper.
...
2. Several parameters (penalty \sigma, shyness threshold \xi, number of smoothing steps s, and cell threshold \lambda) are explored empirically. While this is reasonable, a more explicit discussion or practical guidelines for choosing these parameters in general problems would be useful for practitioners.
    > We added a dedicated subsection "Parameter selection and practical guidelines" in Section 3, providing explicit recommendations for key parameters based on our empirical results.
